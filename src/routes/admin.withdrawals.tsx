...
function AdminWithdrawals() {
  ...
  const list = data.withdrawals.filter((w: any) =>
    filter === "all" ? true : String(w.status || "").toLowerCase() === filter,
  );

  const act = async (id: string, status: "approved" | "rejected") => {
    setBusy(id);
    const r: any = await api("updateWithdrawal", { id, status });
    setBusy(null);
    if (r?.success) {
              ...
              >
                <div className="min-w-0">
                  <p className="font-extrabold">
                    {w.userName || w.userEmail} · {money(w.amount)}
                  </p>
                  <p className="text-xs text-muted-foreground">UPI/Bank: {w.upi}</p>
                  <p className="text-xs text-muted-foreground">{dateTxt(w.createdAt)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase bg-card ${statusClass(w.status)}`}
                  >
                    {w.status}
                  </span>
